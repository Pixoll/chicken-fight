        ...
        public void ReceiveImpact(HurtboxCharacteristics characteristics)
        {
            switch (characteristics.Type)
            {
                case HurtboxCharacteristics.ImpactType.Punch:
                    ...
                    {
                        _punchReceiver.ApplyPunchKnockback(characteristics.GetOriginPosition(), characteristics.Knockback);
                    }
                    break;

                case HurtboxCharacteristics.ImpactType.Environmental:
                    Debug.Log($"<color=yellow>[ImpactManager] Daño ambiental recibido: {characteristics.Damage}</color>");
